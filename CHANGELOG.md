...
- The prompt hook computed the snapshot age with `stat -f`, which on Linux
  reports the filesystem, not the file; now `date -r`, which works on both.
...
- `.agent/VERSION` said 0.4.2 in the 0.5.0 release; corrected.
...
- Step five asks three open questions one at a time (where's your inbox,
  are you mobile a lot, what are you on), then makes one suggestion with
  an example.
