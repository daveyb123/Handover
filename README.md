...
## A to-do list for a small team that lives in plain text files and is run by talking to an AI assistant instead of clicking around an app. Every task and every decision carries its *why*, so whoever picks it up gets the reasoning, not just the job.
